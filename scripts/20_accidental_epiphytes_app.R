...
obs_sf <- fread(file.path(GITHUB_RAW_BASE,"inat_observations.csv")) %>% as_tibble() %>% 
  reframe(latitude, longitude, id, taxon_id, uri, taxon.name, taxon.common, family_name,
          ...
          EpiCount=EpiCount_noNA, GrowingSite,
...
last_updated_text <- tagList(
  HTML(paste0("Last updated on ", d, " by Marie Hoensbroech.<br>")),
  tags$span(
    style = "color:#555;",
    "Something's not working or you have ideas for improvements? ",
    tags$a(
      href   = "https://github.com/MarieHoensbroech/accidental-epiphytes/issues",
      target = "_blank", rel = "noopener", "Report it"
    ),
    " or ",
    tags$a(href = "mailto:marie.hoensbroech@uni-oldenburg.de", "email me!")
  )
)


#____________________----------------
# SITE SUMMARY POP-UP with video-------------------------------------------
site_summary_html <- function(site_key) {
  ...
  n_sp  <- if (nrow(obs_here) > 0) dplyr::n_distinct(obs_here$taxon_id)  else 0
  n_epis  <- if (nrow(obs_here) > 0) sum(obs_here$EpiCount)  else 0
  ...
  # If no poster is listed by API, try conventional cover.jpg (ok if 404; it’s just an <img> attr)
  if (is.na(poster)) poster <- paste0(raw_base, "cover.jpg")
  
  # Build the video HTML: only if there's actually an MP4 in the folder
  if (!is.na(vid_mp4)) {
    ...
    video_html <- paste0(
      ...
      "<div style='font-size:11px; color:#666;'>No MP4 available in this folder.</div>"
    )
  }
 ...
 paste0(
    ...
    "<b>Trees per DBH category (cm):</b> ", dbh_html, "<br/>",
    
    "We found ", format(n_epis, big.mark = ","), " <b>",
    ifelse(n_epis == 1, "accidental epiphyte individual", "accidental epiphyte individuals"), "</b> ",
    
    "of <b>", format(n_sp, big.mark = ","), " ",
    ifelse(n_sp == 1, "species", "species"), "</b> on <b>",
    
    format(n_hosts, big.mark = ","), "</b> ",
    ifelse(n_hosts == 1, "host tree", "host trees"), " of species: ",
    "<i>", htmltools::htmlEscape(host_species_str), "</i>", "<br/>",
    
    "<b>Area (ha):</b> ", round(area_val, 3), ifelse(used_mean, " (imputed mean)", ""), "<br/>",
    
    "<b>Dominant tree species:</b> <i>", htmltools::htmlEscape(dom_list), "</i> <br/>",
...
ui <- fluidPage(
  ...
  tags$head(
   ...
   .obs-details h5, .obs-details h6 {
   margin: 0 !important;    margin-top: 1px !important;
   margin-bottom: 1px !important;   line-height: 0.8 !important;
   }
  "))
  ),
  ...
  tags$head(
    tags$meta(
      ...
      content = paste(
        "default-src 'self' blob: data: https: mailto:;",  # <- added mailto:
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      width = 2,
      actionButton("reset_map", "Reset map", icon = icon("map")),
...
server <- function(input, output, session) {
  ...
  observeEvent(TRUE, {
    ...
    showModal(modalDialog(
      title = "Welcome aboard!", easyClose = TRUE, footer = NULL, size = "l",
      HTML(paste0(
  ...
  observe({ isolate({ session$sendCustomMessage("resetFilters", "All") }) })
  observe({ updateSelectizeInput(session, "taxon_filter", choices = NULL, selected = "All", server = TRUE) })
  ...
  observeEvent(input$reset_filters, {
    updateSelectizeInput(session, "grade_filter",
                         choices = c("All", sort(unique(obs_shiny$quality_grade))), selected = "All")
    updateSelectizeInput(session, "user_filter",
                         choices = c("All", sort(unique(obs_shiny$user_login))),   selected = "All")
    updateSelectizeInput(session, "rank_filter",
                         choices = c("All", sort(unique(obs_shiny$taxon.rank))),   selected = "All")
    updateSelectizeInput(session, "family_filter",
                         choices = c("All", sort(unique(obs_shiny$family_name))),  selected = "All")
    updateSelectizeInput(session, "taxon_filter",
                         choices = c("All", sort(unique(obs_shiny$taxon.name))),   selected = "All")
  })
  ...
  filtered_obs <- reactive({
    data <- obs_shiny %>% dplyr::ungroup()
    if (input$grade_filter  != "All") data <- dplyr::filter(data, quality_grade == input$grade_filter)
    if (input$user_filter   != "All") data <- dplyr::filter(data, user_login   == input$user_filter)
    if (input$rank_filter   != "All") data <- dplyr::filter(data, taxon.rank   == input$rank_filter)
    if (input$family_filter != "All") data <- dplyr::filter(data, family_name  == input$family_filter)
    if (input$taxon_filter  != "All") data <- dplyr::filter(data, taxon.name   == input$taxon_filter)
    data
  })
  
  # --- MAP: "All observations" ---------------------
  output$map <- renderLeaflet({
    req(input$grade_filter, input$user_filter, input$rank_filter, input$family_filter, input$taxon_filter)
    
    # Always draw full set as background
    coords_all <- sf::st_coordinates(obs_shiny)
    
    leaflet(data = obs_shiny) %>%
      ## Basemaps--------------
      ...
      addMapPane("sitesPane",  zIndex = 420) %>%
      addMapPane("tracksPane", zIndex = 410) %>%
      addMapPane("obsPane",    zIndex = 430) %>%
      ## ALL observations (dimmed background, always visible)----------------
      addCircleMarkers(
        data = obs_shiny,
        lng = as.numeric(coords_all[,1]),
        lat = as.numeric(coords_all[,2]),
        layerId = ~as.character(id),
        radius = 5,
        color  = ~col,
        fillOpacity = 0.1,   
        stroke = TRUE, weight = 1, opacity = 0.2,
      ...
      addPolylines(
        ...
        labelOptions = labelOptions(
          sticky = TRUE, direction = "auto", opacity = 0.8,
          textsize = "12px", offset = c(0, -5)
        ),
        color = "cadetblue", weight = 3, opacity = 0.7,
  ...
  # Filtered highlight -----------
  #(two-ring glow). Draw ONLY when set is narrowed 
  observe({
    data <- filtered_obs()
    
    # If filters match the full set (or empty), do NOT draw highlight
    if (is.null(data) || nrow(data) == 0 || nrow(data) == nrow(obs_shiny)) {
      leafletProxy("map") %>% clearGroup("Filtered highlight")
      return()
    }
    ...
    leafletProxy("map") %>%
      clearGroup("Filtered highlight") %>%
      ## Outer halo ---------------
      #non-interactive; never steals clicks
      addCircleMarkers(
        data = data,
        lng = as.numeric(coords[,1]),
        lat = as.numeric(coords[,2]),
        layerId = ~paste0("filt_halo_", id),
        radius = 12, color = "white", fillColor = "white",
        fillOpacity = 0.7, opacity = 0.8, stroke = TRUE, weight = 4,
        group  = "Filtered highlight",
        options = pathOptions(pane = "obsPane", interactive = FALSE)
      ) %>%
      ## Inner core ------------
      #clickable; strip prefix in click handler
      addCircleMarkers(
        data = data,
        lng = as.numeric(coords[,1]),
        lat = as.numeric(coords[,2]),
        layerId = ~paste0("filt_core_", id),
        radius = 7, color = ~col, fillColor = ~col,
        fillOpacity = 0.7, opacity = 0.8, stroke = TRUE, weight = 2,
        group  = "Filtered highlight",
        options = pathOptions(pane = "obsPane")
      )
  })
  
  # ---- Observation Details Pane ------------
  output$obs_details <- renderUI({
    obs.clicked <- clicked_obs_data()
    req(!is.null(obs.clicked), is.data.frame(obs.clicked), nrow(obs.clicked) > 0)
    
    # --- Always define the carousel id BEFORE any CSS/JS uses it
    carousel_id <- paste0("photoCarousel_", as.character(obs.clicked$id[1]))
    
    # --- Helper: force iNat size token to 'medium'
    normalize_inat_size <- function(u) {
      gsub("/(square|small|medium|large|original)/", "/medium/", u, perl = TRUE)
    }
    ...
    # --- Observation details----------
    tagList(
      h6(tags$a(href = unique(obs.clicked$uri)[1], "View on iNaturalist", target = "_blank", rel = "noopener")),
      ui_carousel,
      h6(strong("Taxon name:"), tags$i(unique(obs.clicked$taxon.name)[1]),
      ...
      h6(strong("Host species:"), tags$i(unique(obs.clicked$TreeSp)[1])),
      ...
      h6(
        strong(paste0("Notes from observer (", unique(obs.clicked$user_login)[1], "):")),
  ...
  # Click event  -------------------
  #overlays only; keep "All observations" intact 
  observeEvent(input$map_marker_click, ignoreInit = TRUE, {
    click <- input$map_marker_click
    clicked_id_raw <- as.character(click$id)
    if (!nzchar(clicked_id_raw)) return()
    
    # Ignore site centroid markers here
    if (startsWith(clicked_id_raw, "site_marker_")) return()
    
    # Strip any filtered overlay prefix; else keep as-is
    clicked_id <- sub("^(filt_core_|filt_halo_)", "", clicked_id_raw)
    
    # Look up in FULL dataset so any drawn marker is clickable
    clicked <- try(dplyr::filter(obs_shiny, id == clicked_id), silent = TRUE)
    if (inherits(clicked, "try-error") || nrow(clicked) == 0) return()
    
    clicked_obs_data(clicked)
    
    clicked_taxon_id <- clicked$taxon_id[1]
    if (is.null(clicked_taxon_id) || is.na(clicked_taxon_id)) return()
    
    selected_taxon(unique(clicked$taxon.name)[1])
    
    # Same-taxon set
    mp <- try(dplyr::filter(obs_shiny, taxon_id == clicked_taxon_id), silent = TRUE)
    if (inherits(mp, "try-error")) mp <- NULL
    matching_points(mp)
    
    # Clear ONLY dynamic overlays; DO NOT touch "All observations" or "Filtered highlight"
    leafletProxy("map") %>%
    ...
    # Ranges ----------------
    obs_range_geom  <- try(sf::st_as_sfc(clicked$obs_range_wkt,  crs = 4326), silent = TRUE)
    inat_range_geom <- try(sf::st_as_sfc(clicked$inat_range_wkt, crs = 4326), silent = TRUE)
    if (!inherits(obs_range_geom, "try-error")  && length(obs_range_geom)  > 0 && !sf::st_is_empty(obs_range_geom)) {
      leafletProxy("map") %>%
        addPolygons(data = obs_range_geom,
                    color = "#F7F056", weight = 2, fillOpacity = 0.4,
                    group = "Ranges", options = pathOptions(pane = "rangesPane"))
    }
    if (!inherits(inat_range_geom, "try-error") && length(inat_range_geom) > 0 && !sf::st_is_empty(inat_range_geom)) {
      leafletProxy("map") %>%
        addPolygons(data = inat_range_geom,
                    color = "#90C987", weight = 2, fillOpacity = 0.3,
                    group = "Ranges", options = pathOptions(pane = "rangesPane"))
    }
    ...
    if (!is.null(mp) && nrow(mp) > 0) {
      leafletProxy("map") %>%
        addCircleMarkers(
          data = mp,
          lng = sf::st_coordinates(mp)[,1],
          lat = sf::st_coordinates(mp)[,2],
          layerId = ~as.character(id),
          radius = 7,
          color = "darkgoldenrod",
          fillOpacity = 0.8,
    ...
    # Selected observation halo -------------
    leafletProxy("map") %>%
      addCircleMarkers(
        data = clicked,
        lng = as.numeric(sf::st_coordinates(clicked)[,1]),
        lat = as.numeric(sf::st_coordinates(clicked)[,2]),
        radius = 10,
  ...
  observeEvent(input$map_marker_click, {
    ...
    pad <- 0.0001
    leafletProxy("map") %>%
      fitBounds(lng1 = as.numeric(bb["xmin"]) - pad,
  ...
  output$selected_taxon_summary <- renderUI({
    ...
    obs.clicked <- clicked_obs_data()
    req(NROW(data) > 0)
    n_obs <- unique(data$n_obs); n_obs <- n_obs[!is.na(n_obs)]
    n_obs <- if (length(n_obs) == 1) n_obs else nrow(data)
    sum_individuals <- unique(data$sum_individuals); sum_individuals <- sum_individuals[!is.na(sum_individuals)]
    ...
    message_base <- if ((n_obs == 1 && sum_individuals > 1) || (n_obs == 1 && sum_individuals == 0)) {
      paste0("<i>", taxon_name, "</i> was observed ", n_obs, " time with a total of at least ", sum_individuals, " individuals.")
    } else if (n_obs == 1 && sum_individuals == 1) {
      paste0("<i>", taxon_name, "</i> was observed ", n_obs, " time with a total of at least ", sum_individuals, " individual.")
    } else {
      paste0("<i>", taxon_name, "</i> was observed ", n_obs, " times with a total of at least ", sum_individuals, " individuals.")
    }
    ...
    message_extra <- if (is_range_missing) {
      ...
      paste0("The ranges of <i>", taxon_name, "</i> as accidentally epiphytic or terrestrial lifeform do not currently overlap.")
    } else {
  ...
  # Reset map ------------
  #clear overlays; keep "All observations" background visible ---
  observeEvent(input$reset_map, {
    clicked_obs(NULL); clicked_obs_data(NULL); selected_taxon(NULL); matching_points(NULL)
    leafletProxy("map") %>%
      clearPopups() %>%
      clearGroup("Selected observation") %>%
      clearGroup("Observations of the same taxon") %>%
      clearGroup("Ranges") %>%
      # Keep background + current filtered highlight as-is (do not clear "All observations")
      showGroup("All observations") %>%
      showGroup("Marie's field work: Field sites") %>%
      showGroup("Marie's field work: Salka Valka's track")
  })
}
...
# https://mariehoensbroech.shinyapps.io/accidental_epiphytes_app/

# library(rsconnect)
# 
# # Path to the directory that contains app script
app_dir <- "C:/Users/Marie/Documents/LatGradientsAccEpis/github/scripts/accidental_epiphytes_explorer/"

options(rsconnect.http.timeout = 600)  # 10 minutes
# # Deploy the app directory (the bundle will include app.R + appdata/*)
rsconnect::deployApp(
  appDir = app_dir,
  appPrimaryDoc = "20_accidental_epiphytes_app.R",
  appName = "accidental_epiphytes_app",
  appFiles = c("20_accidental_epiphytes_app.R") # Only include this file
)
