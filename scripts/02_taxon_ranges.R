...
for (i in seq_along(taxon_ids)) {
  id <- taxon_ids[i]
  msg("  [%d/%d] taxon_id=%s", i, length(taxon_ids), id)
...
# . ----------------------
# 3) Calculate % overlap ----------------------------------
# reduce computational intensity by setting precision
inat_clean <- inat_ranges_sf  %>% 
  ...
  select(taxon_id,geometry,area_m2)

obs_clean <- obs_ranges_sf  %>% 
  filter(taxon_id%in%inat_clean$taxon_id) %>% 
  st_set_precision(100) %>% 
  st_transform(54009) %>% 
  group_by(taxon_id) %>%
  summarise(geometry = st_union(geometry), .groups = "drop") %>%
...
range_overlap <- tibble::tibble(
  ...
  area_inat_m2 = inat_clean$area_m2,
  area_obs_m2  = obs_clean$area_m2,
...
# Loop by taxon to compute overlap area
for (k in seq_len(nrow(range_overlap))) {
  tx <- range_overlap$taxon_id[k]
  
  # Extract geometries (may be empty / missing)
  gi <- inat_clean$geometry[inat_clean$taxon_id == tx]
  go <- obs_clean$geometry[obs_clean$taxon_id == tx]
  
  have_inat <- length(gi) == 1
  have_obs  <- length(go) == 1
  
  if (!have_inat && !have_obs) {
    range_overlap$status[k] <- "missing_both"
  }
  
  # Areas (handle missing)
  ai <- if (have_inat) st_area(gi[[1]]) else units::set_units(0, m^2)
  ao <- if (have_obs)  st_area(go[[1]]) else units::set_units(0, m^2)
  
  range_overlap$area_inat_m2[k] <- as.numeric(ai)
  range_overlap$area_obs_m2[k]  <- as.numeric(ao)
  
  if (!have_inat) {
    range_overlap$status[k] <- "no_inat"
    range_overlap$area_overlap_m2[k] <- 0
    range_overlap$pct_inat[k] <- NA_real_
    range_overlap$pct_obs[k]  <- if (as.numeric(ao) > 0) 0 else NA_real_
  }
  if (!have_obs) {
    range_overlap$status[k] <- "no_obs"
    range_overlap$area_overlap_m2[k] <- 0
    range_overlap$pct_obs[k]  <- NA_real_
    range_overlap$pct_inat[k] <- if (as.numeric(ai) > 0) 0 else NA_real_
  }
  
  # Both present -> compute intersection area (only once per taxon)
  ov <- tryCatch(
    st_intersection(gi[[1]], go[[1]]),
    error = function(e) NULL
  )
  aov <- if (is.null(ov) || length(ov) == 0) units::set_units(0, m^2) else st_area(ov)
  
  range_overlap$area_overlap_m2[k] <- as.numeric(aov)
  
  # Percentages with zero-guards
  ain <- as.numeric(ai); aob <- as.numeric(ao); aol <- as.numeric(aov)
  range_overlap$pct_inat[k] <- if (ain > 0) aol / ain * 100 else NA_real_
  range_overlap$pct_obs[k]  <- if (aob > 0) aol / aob * 100 else NA_real_
...
# View taxa without overlap:
dplyr::filter(range_overlap, status == "no_overlap") %>%
