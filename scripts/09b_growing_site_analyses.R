...
suppressPackageStartupMessages({
  library(tidyverse)     # pulls: ggplot2, dplyr, tidyr, readr, , forcats, purrr, tibble
  library(data.table)
  library(ggpubr)
  library(viridis)
  library(scales)
...
OUT_DIR_HOST <- "outputs/10_hosttree"
...
as_draws_df(fit_obs_host) %>%
  dplyrselect(dplyrstarts_with("b_")) %>%
  tidyrpivot_longer(dplyreverything(), names_to = "bterm", values_to = "draw") %>%
  dplyrmutate(term = str_remove(bterm, "^b_")) %>%
  dplyrgroup_by(term) %>%
  dplyrsummarise(
    mean    = mean(draw),
    ci_low  = quantile(draw, 0.025),
    ci_high = quantile(draw, 0.975),
    pd      = as.numeric(bayestestRp_direction(draw)),
    rope_perc = bayestestRrope(draw, range = c(-log(1.1), log(1.1)))$ROPE_Percentage[1],
  ...
  dplyrarrange(term) %>%
  dplyrmutate(dplyracross(where(is.numeric), ~ round(., 3))) %>%
  fwrite(file.path(OUT_DIR_DBH,"obs.count~host_fixed_effects_summary.csv"))
...
bind_cols(obs_z, pred_z) %>%
  data.tablefwrite(file.path(OUT_DIR_DBH,"obs.count~dbh+setting_zero_rates.csv"))
## 2.2) Species counts -----------
fit_spp_host <- brm(
...
# Bind and write
bind_cols(obs_z, pred_z) %>%
  data.tablefwrite(file.path(OUT_DIR_HOST,"spp.count~host_zero_rates.csv"))
...
df_genus_counts <- inat_field %>%
  ...
  filter(!is.na(host_genus), host_genus != "", !is.na(epi_count)) %>%
...
df_genus_counts_lvl <- df_genus_counts %>% 
  ...
  mutate(host_genus_2 = if_else(n <= 2,
                                ...
                                host_genus) %>% 
           forcatsfct_inorder() %>%
           forcatsfct_relevel("Other hosts", after = Inf)) %>% 
...
df_counts <- inat_field %>%
  reframe(
    ...
    dominanttree1, dominanttree2,dominanttree3, dominanttree4
  ) %>%
  filter(!is.na(site), !is.na(host_genus), host_genus != "", !is.na(individuals)) %>%
  group_by(site, host_genus,dominanttree1,dominanttree2,dominanttree3,dominanttree4) %>%
...
full_grid  <- tidyrcrossing(all_sites, all_hosts)
...
agg <- agg %>%
  mutate(
    ...
    dominant_tree = forcatsfct_reorder(dominant_tree, n_sites, .desc = TRUE)
  )
...
inat_field %>%
  distinct(
    host_genus=tree_genus,tree_sp,id) %>% view()
