...
OUT_DIR_LATITUDE  <- "outputs/figures/06_latitude_analyses"
...
sites_map <- sites_clean %>%
  ...
  reframe(
    lon  = first(lon),
    lat  = first(lat),
    n_sp = suppressWarnings(first(n_sp_per_site)),
    n_epis = suppressWarnings(first(n_epis_per_site)),
    Type,
    .groups = "drop"
  ) %>% 
  drop_na(Type)
...
p_site_raw <- ggplot() +
  ...
  geom_point(
    data = sites_map,
    aes(lon, lat, size = n_epis, color = n_sp, shape = Type),
...
ggsave(file.path(OUT_DIR, "sites_map_raw.jpg"),
...
sites_eff <- my_sites %>%
  mutate(
    lon = as.numeric(SiteLon),
    lat = as.numeric(SiteLat),
    NumberTrees = as.numeric(NumberTrees),
    n_epis_cat = suppressWarnings(as.numeric(n_epis_per_site_cat)),
    n_sp_cat   = suppressWarnings(as.numeric(n_sp_per_site_cat)),
    n_epis_site= suppressWarnings(as.numeric(n_epis_per_site)),
    n_sp_site  = suppressWarnings(as.numeric(n_sp_per_site))
  ) %>%
  group_by(Site) %>%
  reframe(
    total_trees = sum(NumberTrees, na.rm = TRUE),
    n_epis = if (any(is.finite(n_epis_site))) n_epis_site[which.max(is.finite(n_epis_site))] else sum(n_epis_cat, na.rm = TRUE),
    n_sp   = if (any(is.finite(n_sp_site)))   n_sp_site[which.max(is.finite(n_sp_site))]     else sum(n_sp_cat,   na.rm = TRUE),
    lon = first(lon),
    lat = first(lat),
    Type
  ) %>%
  filter(is.finite(lat), is.finite(lon), !(lat == 0 & lon == 0), total_trees > 0) %>%
  mutate(
...
p_site_eff <- ggplot() +
  ...
  geom_point(
    data = sites_eff,
    aes(lon, lat, size = obs_per10 + eps2, color = spp_per10, shape = Type),
...
ggsave(file.path(OUT_DIR, "sites_map_effort_adjusted.jpg"),
...
p_lat_species <- 
  ggplot(inat_by_site, aes(y = lat, x = n_tax_site,col=type)) +
...
p_lat_obs <- 
  ggplot(inat_by_site, aes(y = lat, x = n_obs_site, col=type)) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_point(size = 6, stroke = 0.4, alpha=0.7) +
  labs(
    x = "Individual count",
...
dominant_species_field <-
  ...
  mutate(taxon.name=if_else(str_detect(taxon.name,"Polypodium")|
                              str_detect(taxon.name,"Polypodiales"),
                            "Polypodium sp.",taxon.name)) %>% 
  group_by(lat_mid5,lon_mid5, type,taxon.name) %>% 
  #filter(taxon.rank%in%c("species","genus","complex")) %>% 
  reframe(sum=sum(count),taxon.name) %>% unique() %>% 
  group_by(lon_mid5,lat_mid5,type) %>% 
  distinct() %>%
  mutate(rank = dense_rank(desc(sum))) %>% 
  group_by(lat_mid5,lon_mid5,type) %>% 
  arrange(rank) %>% 
  slice_head(n=10) %>% 
  arrange(desc(lat_mid5), taxon.name) %>%
  ungroup() %>% 
...
p_legend_field <- ggplot(dominant_species_field, aes(x = type, y = sum, fill = taxon.name)) +
  geom_col(position = "fill") +
...
make_bar <- function(df) {
  df %>% 
    mutate(type=if_else(type=="Forest","F","W")) %>% 
    
    ggplot(aes(x = type, y = sum, fill = taxon.name)) +
    geom_col(position = "fill") +
...
df_segments_field <- dominant_species_field %>%
  group_by(lat_mid5, type, taxon.name) %>%
  summarise(sum = sum(sum, na.rm = TRUE), .groups = "drop") %>%
  group_by(lat_mid5, type) %>%
  mutate(
    total = sum(sum, na.rm = TRUE),
    prop  = if_else(total > 0, sum / total, 0)
  ) %>%
  ...
  # Order segments by taxon for reproducibility (or pick another order)
  arrange(type, lat_mid5, taxon.name) %>%
  group_by(type, lat_mid5) %>%
  # Build cumulative horizontal allocation [0..1]
  mutate(
    xmin = cumsum(dplyr::lag(prop, default = 0)),
    xmax = cumsum(prop)
