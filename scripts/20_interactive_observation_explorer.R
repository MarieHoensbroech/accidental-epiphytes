...
# ------------------------------- Load data ------------------------------------
range_lookup <- fread("data/Spatial/range_lookup.csv") %>% as_tibble() %>% 
  mutate(taxon_id=as.character(taxon_id))

obs_sf <- fread("data/processed/inat_observations.csv") %>% as_tibble() %>% 
...
server <- function(input, output, session) {
  ...
  ##### Output 4: Summary: observation and individual count #####
